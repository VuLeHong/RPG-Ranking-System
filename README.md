# Management System Website

This is a web application built for managing tasks, user profiles, project details, rankings, and comparisons. It leverages **Express** for the backend, **React** for the frontend, and **MongoDB** for the database to provide a seamless experience.

## Features

- **Login Page**: Secure authentication for users.
- **Profile Page**: Display and analyst user profile information and stats.
- **Task Management Page**: Show and update tasks.
- **Ranking Page**: Compare user rankings and achievements.
- **Project Management Page**: Overview of all projects, including details.
- **Comparison System**: Ability to compare rankings between users stats.

## Technologies Used

- **Frontend**: React
- **Backend**: Express
- **Database**: MongoDB

## Screenshots

### Login Page
![Login Page](https://drive.google.com/file/d/1jwWJpt6z32qUqNTsP2_u5kAcRIh_P-vE/view?usp=drive_link)

### Profile Page
![Profile Page](https://drive.google.com/file/d/1MW1gQyUkNaCnzhMt6lrBYfoWfKGvRc5m/view?usp=drive_link)

### Task Management Page
![Task Management Page](https://drive.google.com/file/d/1fhMSUkTdNXbLKgVo7ZvPuE1T-DaHrSVj/view?usp=drive_link)

### Ranking Page
![Ranking Page](https://drive.google.com/file/d/1g-prW0HX7Mlf0JErmzFvXqxLMEd4riIT/view?usp=drive_link)

### Compare System in Ranking
![Compare System in Ranking](https://drive.google.com/file/d/1P7razJrS_-7j6s1tqGaplviHSMA7TlKr/view?usp=drive_link)

### Project Management Page
![Project Management Page](https://drive.google.com/file/d/1arONs1DK5K02kpiWQq18_b5o4QSXu9JD/view?usp=drive_link)

### Project Create Modal
![Project Detail](https://drive.google.com/file/d/1fEk4YOU2B0KsvqQM9ZHK0V8ISqVqb2Fw/view?usp=drive_link)

### Task Create Modal
![Project Detail](https://drive.google.com/file/d/1fEk4YOU2B0KsvqQM9ZHK0V8ISqVqb2Fw/view?usp=drive_link)





